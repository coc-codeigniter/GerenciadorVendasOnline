...
if($pedidos){
   ...
   foreach($pedidos as $pedido){
     ...
     $tbl .='<tr><td>'.$pedido->id.'</td><td>'.$pedido->nome.'</td><td>'.format_data_br($pedido->data_pedido).'</td><td>R$ '.number_format($pedido->total,2,',','.').'</td><td>'.$pedido->nome_usuario.'</td><td>'.$pedido->credor.'</td><td>'.$pedido->parcelas.'</td><td><a href="#pedidoFluxo$'.$pedido->id.'" class="fluxoPedido" ><i class="icon-chevron-right"></i></a> | <a href="#faturarFluxo$'.$pedido->id.'" class="faturarPedido" ><i class="icon-check"></i></a></td></tr>'; 
   }
   $tbl .='</tbody></table>';
   echo $tbl;
   }else{
