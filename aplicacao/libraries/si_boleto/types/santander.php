...
class Santander
{
    ...
    var $codigobanco = 033; //Antigamente era 353
    var $codigo_banco_com_dv; //geraCodigoBanco($codigobanco);
    var $nummoeda = 9;
    var $fixo = 9; // Numero fixo para a posição 05-05
    var $ios = 0; // IOS - somente para Seguradoras (Se 7% informar 7, limitado 9%)
    var $fator_vencimento; //= fator_vencimento($dadosboleto["data_vencimento"]);
    //valor tem 10 digitos, sem virgula
    var $valor; // = formata_numero($dadosboleto["valor_boleto"],10,0,"valor");
    //Modalidade Carteira
    var $carteira; //= $dadosboleto["carteira"];
    //codigocedente deve possuir 7 caracteres
    var $codigocliente; // = formata_numero($dadosboleto["codigo_cliente"],7,0);
    //nosso número (sem dv) é 11 digitos
    var $nnum; //= formata_numero($dadosboleto["nosso_numero"],7,0);
    //dv do nosso número
    var $dv_nosso_numero; // = modulo_11($nnum,9,0);
    // nosso número (com dvs) são 13 digitos
    var $nossonumero; // = "00000".$nnum.$dv_nosso_numero;
    var $vencimento; // = $dadosboleto["data_vencimento"];
    var $vencjuliano; // = dataJuliano($vencimento);
    // 43 numeros para o calculo do digito verificador do codigo de barras
    private $barra; // = "$codigobanco$nummoeda$fator_vencimento$valor$fixo$codigocliente$nossonumero$ios$carteira";
    //$barra = "$codigobanco$nummoeda$fixo$codigocliente$nossonumero$ios$carteira";
    private $dv; // = digitoVerificador_barra($barra);
    // Numero para o codigo de barras com 44 digitos
    private $linha; /// = substr($barra,0,4) . $dv . substr($barra,4);
    private $sacado  = array();
    private $especie = 'R$';
    function __construct(){
        $this->type = Type::type_instance();
    }
    
    function setDataBoleto(array $options = array())
    {
        $this->carteira = $options['carteira'];
        $this->valor = $this->formata_numero($options['valor'], 10, 0, 'valor');
        $this->codigo_banco_com_dv = $this->geraCodigoBanco($this->codigobanco);
        $this->fator_vencimento = $this->fator_vencimento($options['data_vencimento']);
        $this->codigocliente = $this->formata_numero($options['codigo_cliente'], 7, 0);
        $this->nnum = $this->formata_numero($options['nosso_numero'], 7, 0);
        $this->dv_nosso_numero = $this->modulo_11($this->nnum);
        $this->nossonumero = "00000" . $this->nnum . $this->dv_nosso_numero;
        $this->vencimento = $options['data_vencimento'];
        ...
        $this->sacado = isset($options['sacado']) ? $options['sacado'] : array();
        $this->cnpj =  $options['cnpj_cpf'];
        $this->cedente = $options['cedente'];
    ...
    function getDataBoleto()
    {
        ...
        $dadosboleto['valor'] = $this->valor;
        $dadosboleto['carteira'] =  $this->carteira;
        $dadosboleto['sacado'] = $this->sacado;
        $dadosboleto['data_vencimento'] = $this->vencimento;
        $dadosboleto['codigo_cliente'] = $this->codigocliente;
        $dadosboleto['especie'] = $this->especie;
        $dadosboleto['cnpj_cpf'] = $this->cnpj;
        $dadosboleto['cedente'] =  $this->cedente;
        return $dadosboleto;
    }
    ...
    function fbarcode($valor)
    {
        ...
        $largo = 3;
        $altura = 50;
        $pathimages = $this->type->ci_config['directory_images_boletos'];
        $barcodes[0] = "00110";
        $barcodes[1] = "10001";
        $barcodes[2] = "01001";
        $barcodes[3] = "11000";
        ...
        for ($f1 = 9; $f1 >= 0; $f1--) {
            for ($f2 = 9; $f2 >= 0; $f2--) {
                ...
                $texto = "";
                for ($i = 1; $i < 6; $i++) {
                    $texto .= substr($barcodes[$f1], ($i - 1), 1) . substr($barcodes[$f2], ($i - 1),
                        1);
                }
                $barcodes[$f] = $texto;
            }
        ...
        //Guarda inicial
        $html = '
    <img src="'.$pathimages.'boletos/p.png" width="'. $fino.'" height='.$altura.' border="0" /><img 
    src="'.$pathimages.'boletos/b.png" width="'.$fino.'" height="'. $altura.'" border="0" /><img 
    src="'.$pathimages.'boletos/p.png" width="'.$fino.'" height='.$altura.' border="0" /><img 
    src="'.$pathimages.'boletos/b.png" width="'. $fino.'" height='.$altura.' border="0" /><img ';
        $texto = $valor;
        if ((strlen($texto) % 2) <> 0) {
            $texto = "0" . $texto;
        ...
        while (strlen($texto) > 0) {
            ...
            for ($i = 1; $i < 11; $i += 2) {
                ...
                $html .= ' src="'.$pathimages.'boletos/p.png" width="'.$f1.'" height="'.$altura.'" border="0" /><img';
                ...
                $html .= '
    src="'.$pathimages.'boletos/b.png" width="'.$f2.'" height="'.$altura.'" border="0"/><img 
    ';
            }
        }
        ...
        // Draw guarda final
        $html .= '
    src="'.$pathimages.'boletos/p.png" width="'.$largo.'" height='.$altura.' border="0" /><img 
    src="'.$pathimages.'boletos/b.png" width="'. $fino.'" height='.$altura.' border="0" /><img 
    src="'.$pathimages.'boletos/p.png" width="1" height="'.$altura.'" border="0"> 
    ';
    return $html;
    } //Fim da função
    ...
    function fator_vencimento($data)
    {
        ...
        $mes = $data[1];
        $dia = $data[0];
        return (abs(($this->_dateToDays("1997", "10", "07")) - ($this->_dateToDays($ano,
            $mes, $dia))));
    }
    ...
    private function _dateToDays($year, $month, $day)
    {
        ...
        return (floor((146097 * $century) / 4) + floor((1461 * $year) / 4) + floor((153 *
            $month + 2) / 5) + $day + 1721119);
    }
   ...
   private function monta_linha_digitavel($codigo)
    {
        ...
        $campo1 = substr($codigo, 0, 3) . substr($codigo, 3, 1) . substr($codigo, 19, 1) .
            substr($codigo, 20, 4);
    ...
    function createBoleto(){
        ...
        $data['codigo_barras']=  $this->fbarcode($data['linha']);
        $this->type->set_var($data);
       $this->type->layout();
